...
const manualCashoutOrRollback = async (req, res) => {
    try {
        const { id, event, operator_id, description, amount, txn_ref_id, user_id, session_token } = req.body;
        ...
        if (getTransaction.length > 0) {
            ...
            if (operator.length > 0) {
                ...
                if (operatorUrl) {
                    ...
                    let amount = event == 'cashout' ? amount : getRollbackTransaction[0].amount;
                    let description = event == 'cashout' ? description : `${amount} Rollback-ed for transaction with reference ID ${txn_ref_id}`;
                    let txn_type = event == 'cashout' ? 1 : 2;
                    let webhookData = { txn_id, amount, txn_ref_id, description, txn_type, token: options.token };
                    ...
                    await axios(requestOptions).then(async (data) => {
                        if (data.status == 200) {
                            if(event === "cashout"){
                                ...
                                await write.query(`INSERT IGNORE INTO transaction (user_id, session_token , operator_id, txn_id, amount,  txn_ref_id , description, txn_type, txn_status) VALUES (?)`, [[]]);
