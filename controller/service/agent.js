...
const addAgent = async (req, res) => {
  try {
    ...
    // const checkQuery = "SELECT * FROM user_credentials WHERE user_id = ?";
    // const [getUser] = await write(checkQuery, [user_id]);
    ...
    if (getUser) {
      ...
      await loadConfig({ loadAll: true , loaduser_credentials: true});
      return res.status(200).send({ status: true, msg: "Agent added successfully" });
...
const agentList = async (req, res) => {
    try {
      ...
      if (isNaN(limit) || isNaN(offset)) {
        ...
        return res.status(200).send({ status: true, msg: "Agents list fetched successfully", data: AgentList });
    } catch (error) {
      return res.status(500).send({ status: false, msg: "Internal Server Error" });
    }
  };
  ...
  const agentChangePassword = async (req, res) => {
    try {
     ...
     const data = await write("UPDATE user_credentials SET password = ? WHERE user_id = ?", [hashedPassword, user_id]);
     console.log(data)
     await loadConfig({ loadAll: true , loaduser_credentials: true});
...
module.exports = {   agentList  , addAgent , agentChangePassword}
