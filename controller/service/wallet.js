...
const updateUserBalance = async (req, res) => {
    try {
        const token = req.headers.token;
        const {  txn_id, amount, txn_ref_id, description, txn_type, ip } = req.body;
        let validateUser;
        try {
            validateUser = JSON.parse(await getRedis(token));
        } catch (err) {
        ...
        const encryptedData = await encryption({ amount, txn_id, description, txn_type, txn_ref_id, ip }, secret);
        const options = {
            method: 'POST',
            url: operatorUrl,
            headers: {
